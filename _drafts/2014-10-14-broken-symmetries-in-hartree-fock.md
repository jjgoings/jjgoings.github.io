...
We want to classify broken symmetry wave function by investigating how they transform under the action of the invariance operators $$ {\hat{g}}&fg=000000$$ constituting the symmetry group $$ {\mathcal{G}}&fg=000000$$ of the spin-free electronic Hamiltonian $$ {\hat{H}}&fg=000000$$ which is equivalent to

$$ \displaystyle \hat{g}\hat{H}\hat{g}^{-1} = \hat{H}, \qquad (\forall \hat{g} \in \mathcal{G}). \ \ \ \ \ (1)&fg=000000$$

This simply means $$ {\hat{H}}&fg=000000$$ is invariant to transformation by $$ {\hat{g}}&fg=000000$$, whatever $$ {\hat{g}}&fg=000000$$ may be. In general, when $$ {|\psi\rangle}&fg=000000$$ is an eigenstate of $$ {\hat{H}}&fg=000000$$, then $$ {\hat{g}|\psi\rangle}&fg=000000$$ is also an eigenstate belonging to the same eigenvalue as $$ {\psi\rangle}&fg=000000$$.  
Now, exact eigenstates of $$ {\hat{H}}&fg=000000$$ can be chosen to be simultaneous eigenstates of the various symmetries in $$ {\mathcal{G}}&fg=000000$$. In contrast, for approximate variational wave functions (e.g. Hartree-Fock) the symmetry requirements represent additional constraints.
...
$$ \displaystyle \hat{h} \left( \begin{array}{cc} \mathbf{A}\_{\alpha\alpha} & \mathbf{A}\_{\alpha\beta} \\ \mathbf{A}\_{\beta\alpha} & \mathbf{A}\_{\beta\beta} \end{array} \right)\hat{h}^{-1} = \left( \begin{array}{cc} \mathbf{A}\_{\alpha\alpha} & \mathbf{A}\_{\alpha\beta} \\ \mathbf{A}\_{\beta\alpha} & \mathbf{A}\_{\beta\beta} \end{array} \right) \ \ \ \ \ (8)&fg=000000$$
...
Since by definition $$ {\hat{A} = e^{(-i\hat{B})} \hat{A} e^{(i\hat{B})}}&fg=000000$$ It suffices to show that the constraints on $$ {\hat{A}}&fg=000000$$ introduced by the symmetry operation are satisfied when $$ {\left[\hat{A},i\hat{B}\right] = 0}&fg=000000$$.  
This is an excellent result, because it means that we can just look at how the $$ {2\times2}&fg=000000$$ matrix $$ {\mathbf{A}}&fg=000000$$ transforms under the Pauli spin matrices which define the SU(2) spin rotation as well as the time-reversal operation (in the time-reversal case, we can use the BCH expansion of the unitary part $$ {\hat{S}\_y}&fg=000000$$ and then absorb the complex conjugation into the commutator expression).
...
$$ {\mathbf{I}}&fg=000000$$. This means there is no symmetry.  
$$ {\Theta}&fg=000000$$. This means we are just symmetric to time-reversal symmetry.  
$$ {K}&fg=000000$$. Only symmetric to complex conjugation.  
$$ {S\_z}&fg=000000$$. Only symmetric to spin rotations about the z-axis.  
$$ {S\_z}&fg=000000$$,$$ {\Theta}&fg=000000$$. Symmetric to both spin rotation about z, as well as time-reversal symmetry.  
$$ {S\_z}&fg=000000$$, $$ {K}&fg=000000$$. Symmetric to spin rotation about z, as well as complex conjugation.  
$$ {S^2}&fg=000000$$. Symmetric to all spin rotations.  
$$ {S^2}&fg=000000$$, $$ {\Theta}&fg=000000$$. Symmetric to all spin rotations and time reversal symmetries.
...
$$ \displaystyle \left( \begin{array}{cc} \mathbf{F}\_{\alpha\alpha} & \mathbf{F}\_{\alpha\beta} \\ \mathbf{F}\_{\beta\alpha} & \mathbf{F}\_{\beta\beta} \end{array} \right) \ \ \ \ \ (15)&fg=000000$$

Where we solve, for each symmetry (or group of symmetries),

$$ \displaystyle \mathbf{h} \left( \begin{array}{cc} \mathbf{F}\_{\alpha\alpha} & \mathbf{F}\_{\alpha\beta} \\ \mathbf{F}\_{\beta\alpha} & \mathbf{F}\_{\beta\beta} \end{array} \right)\mathbf{h}^{-1} = \left( \begin{array}{cc} \mathbf{F}\_{\alpha\alpha} & \mathbf{F}\_{\alpha\beta} \\ \mathbf{F}\_{\beta\alpha} & \mathbf{F}\_{\beta\beta} \end{array} \right) \ \ \ \ \ (16)&fg=000000$$

where $$ {\mathbf{h}}&fg=000000$$ is the generator of the symmetry.

**$$ {\mathbf{I}}&fg=000000$$. No symmetry.**

In this case, our transformation on $$ {\mathbf{F}}&fg=000000$$ is rather simple. It looks like

$$ \displaystyle \mathbf{I} \left( \begin{array}{cc} \mathbf{F}\_{\alpha\alpha} & \mathbf{F}\_{\alpha\beta} \\ \mathbf{F}\_{\beta\alpha} & \mathbf{F}\_{\beta\beta} \end{array} \right)\mathbf{I}^{-1} = \left( \begin{array}{cc} \mathbf{F}\_{\alpha\alpha} & \mathbf{F}\_{\alpha\beta} \\ \mathbf{F}\_{\beta\alpha} & \mathbf{F}\_{\beta\beta} \end{array} \right) \ \ \ \ \ (17)&fg=000000$$
...
**$$ {K}&fg=000000$$. Complex conjugation symmetry.**
...
**$$ {S\_z}&fg=000000$$,$$ {\Theta}&fg=000000$$. Rotation about spin z-axis and time reversal.**
...
Two things worth mentioning here: first, technically $$ {S\_z}&fg=000000$$ is already a part of the total spin rotation group, so it's a little weird to separate them into $$ {S^2}&fg=000000$$ and $$ {S\_z}&fg=000000$$, but we understand this as drawing the distinction that you can be symmetric to $$ {S\_z}&fg=000000$$ but not $$ {S^2}&fg=000000$$.

If you are invariant to $$ {S^2}&fg=000000$$, though, you will be invariant to $$ {S\_z}&fg=000000$$.

Second, while $$ {S^2}&fg=000000$$ does technically have an operator representation, it is not a symmetry operation. Think about the form of the operator $$ {S^2}&fg=000000$$: it's essentially the identity, right? So when we say invariant with respect to $$ {S^2}&fg=000000$$, what we really mean is that we are invariant to the whole spin rotation group, $$ {S\_x,S\_y,S\_z}&fg=000000$$.
...
You can show this using the Jacobi identity. Say we are looking the invariance of $$ {F}&fg=000000$$ with respect to generators $$ {A}&fg=000000$$,$$ {B}&fg=000000$$,and $$ {C}&fg=000000$$, and $$ {[A,B] = iC}&fg=000000$$ (like our spin matrices do). We want to show = [F,B] = 0 \implies [F,C] = 0 The Jacobi identity tells us ] + [B, [F, A]] + [F, [A, B]] = 0 Now, by definition the first two terms are zero, and we can evaluate the commutator of $$ {[A,B] = iC}&fg=000000$$, which means if $$ {[F,A] = [F,B] = 0}&fg=000000$$, then it must follow that $$ {[F,C] = 0}&fg=000000$$ as well (the imaginary in front doesn't make a difference; expand to see).

That being said, we can evaluate invariance with respect to all spin axes by using the results of $$ {S\_z}&fg=000000$$ and applying the generator $$ {S\_x}&fg=000000$$ defined by Pauli matrix $$ {\sigma\_2}&fg=000000$$ to it, where

$$ \displaystyle \sigma\_2 = \left( \begin{array}{cc} 0 & 1 \\ 1 & 0 \end{array} \right) \ \ \ \ \ (29)&fg=000000$$

Applying this gives

$$ \displaystyle \left( \begin{array}{cc} 0 & 1 \\ 1 & 0 \end{array} \right)\left( \begin{array}{cc} \mathbf{F}\_{\alpha\alpha} & \mathbf{0} \\ \mathbf{0} & \mathbf{F}\_{\beta\beta} \end{array} \right)\left( \begin{array}{cc} 0 & 1 \\ 1 & 0 \end{array} \right) = \left( \begin{array}{cc} \mathbf{F}\_{\beta\beta} & \mathbf{0} \\ \mathbf{0} & \mathbf{F}\_{\alpha\alpha} \end{array} \right) \ \ \ \ \ (30)&fg=000000$$

Which means that $$ {\mathbf{F}\_{\alpha\alpha} = \mathbf{F}\_{\beta\beta}}&fg=000000$$, or

$$ \displaystyle \mathbf{F} = \left( \begin{array}{cc} \mathbf{F}\_{R} & \mathbf{0} \\ \mathbf{0} & \mathbf{F}\_{R} \end{array} \right), \qquad (\mathbf{F}\_{R} \in \mathbb{C}) \ \ \ \ \ (31)&fg=000000$$
...
**$$ {S^2}&fg=000000$$,$$ {S\_z}&fg=000000$$,$$ {\Theta}&fg=000000$$,$$ {K}&fg=000000$$. All spin rotations and time reversal.**

Given the results we just obtained above, and understanding that time reversal contains the $$ {S\_y}&fg=000000$$ operator, we only need to take the previous results and make them invariant to complex conjugation. This is very simple, and we see that

$$ \displaystyle K\left( \begin{array}{cc} \mathbf{F}\_{R} & \mathbf{0} \\ \mathbf{0} & \mathbf{F}\_{R} \end{array} \right)K = \left( \begin{array}{cc} \mathbf{F}\_{R}^{\*} & \mathbf{0} \\ \mathbf{0} & \mathbf{F}\_{R}^{\*} \end{array} \right) \ \ \ \ \ (32)&fg=000000$$
